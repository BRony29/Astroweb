...
class BBCode
{
  ...
  public function __construct()
  {
    ...
    // Remplacer [sm*] par <img src="/BBcdode/smiles/sm*.png alt="smiley">.
    $this->bbcode_table["/\[sm(.*?)]/is"] = function ($match) {
      return "<img src=\"/public/BBCode/smiles/sm$match[1].png\" alt=\"smiley\">";
    };

    // Remplacer [br] par <br />.
    $this->bbcode_table["/\[br]/is"] = function () {
      return "<br />";
    };
    ...
    $this->bbcode_table["/\[sup\](.*?)\[\/sup\]/is"] = function ($match) {
      return "<sup>$match[1]</sup>";
    };


    // Replace [sub]...[/sub] with <sup>...</sup>
    $this->bbcode_table["/\[sub\](.*?)\[\/sub\]/is"] = function ($match) {
      return "<sub>$match[1]</sub>";
    };
    ...
    $this->bbcode_table["/\[quote=\"([^\"]+)\"\](.*?)\[\/quote\]/is"] = function ($match) {
      return "$match[1] a écrit: <blockquote><p>$match[2]</p></blockquote>";
    };
    ...
    $this->bbcode_table["/\[img\](.*?)\[\/img\]/is"] = function ($match) {
      return "<img class='img-fluid' alt=\"$match[1]\" src=\"$match[1]\"/>";
    };
